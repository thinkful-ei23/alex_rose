...
function validateKeys(object, expectedKeys) {
  ...
  // check object keys 
  //compare each object key to all expectedKeys 
  //check if object key is not in expectedKeys, return false
  //check if object has all expectedKeys, return false 
 
    if (Object.key(object).length !== extendedKey.length) {
      return false; 
    }
    for (let i = 0; i < expectedKeys.length; i++) {
      if (!Object.keys(object).find(function (argument) {
        return argument === expectedKeys[i];
        return false; 
      });
      return true;
    }  
